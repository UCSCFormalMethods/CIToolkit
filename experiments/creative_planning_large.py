...
class SimpleOMTConstrainedPlanning():
    ...
    def create_label_constraint(self):
        ...
        label_1_constraint = z3.And(z3.ULE(1, used_vehicles), z3.ULE(used_vehicles, 2)) == (label_var == 0)

        label_2_constraint = z3.And(z3.ULE(3, used_vehicles), z3.ULE(used_vehicles, 4)) == (label_var == 1)

        label_3_constraint = z3.And(z3.ULE(5, used_vehicles), z3.ULE(used_vehicles, 5)) == (label_var == 2)
...
def plan(vehicles, pois):
    ...
    label_map = {"1-2_Vehicles": 0, "3-4_Vehicles": 1, "5_Vehicles": 2}
    ...
    word_prob_bounds = {label:(0,5e-1) for label in label_func.labels}
    ...
    samples = {i:improviser.improvise() for i in range(20)}
    ...
    for i, sample in samples.items():
        ...
        for key, val in sample.items():
            if key[:12] == "SortieCount_":
                if val != 0:
                    assert val == 1
                    sample_val = key[12:].split("_")
                    sample_list.append(sample_val)
                    print("  " + str(sample_val[0]) + " -> " + sample_val[1])
            else:
                print(key, val)
...
def import_data(job_file, results_file, target_env):
    ...
    pois = [Poi("Poi_" + str(i), i, 1) for i in range(5)]
    ...
    # Duplicate vehicles
    vehicle_list = list(vehicle_dict.values())
    ...
    for veh in vehicle_list:
        assert list(veh.feasible_per_poi.keys()) == list(range(5))
        assert list(veh.survey_per_sortie_per_poi.keys()) == list(range(5))
        assert list(veh.travel_time_per_sortie_per_poi.keys()) == list(range(5))

    return vehicle_list, list(pois)

if __name__ == "__main__":
    random.seed("foobrgbar")
