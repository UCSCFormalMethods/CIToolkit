...
class Alphabet:
    ...
    def create_alphabet(alphabet_input: Any) -> Alphabet:
        ...
        if isinstance(alphabet_input, Alphabet):
            ...
            raise ValueError(
                f"Cannot create an alphabet from type {type(alphabet_input)}"
            )
...
class AbstractSpec(Spec):
    ...
    def __init__(self, spec_1: Spec, spec_2: Spec | None, operation: SpecOp):
        ...
        if (spec_2 is not None) and (spec_1.alphabet != spec_2.alphabet):
            raise ValueError(
                "Cannot perform operations on specifications with incompatible"
                " alphabets."
            )
        ...
        if (operation in [SpecOp.UNION, SpecOp.INTERSECTION]) and (
            not isinstance(spec_1, Spec) or not isinstance(spec_2, Spec)
        ):
            raise ValueError(
                "The union and intersection operations require two specifications as"
                " input."
            )

        if (operation == SpecOp.NEGATION) and (
            not isinstance(spec_1, Spec) or spec_2 is not None
        ):
            raise ValueError(
                "The negation operation requires exactly one specification as input."
            )
    ...
    def accepts(self, word: tuple[str, ...]) -> bool:
        ...
        if self.operation == SpecOp.UNION:
            assert self.spec_2 is not None
            ...
            raise NotImplementedError(f"{self.operation} is not currently supported.")
    ...
    def language_size(self, *args, **kwargs) -> int:
        ...
        raise NotImplementedError(
            "Computation if language_size for abstract specifications of types"
            f" '{spec_1_explicit.__class__.__name__}' and"
            f" '{spec_2_explicit.__class__.__name__}' with operation"
            f" '{self.operation}' is not supported."
        )
    ...
    def sample(self, *args, **kwargs) -> Any:
        ...
        raise NotImplementedError(
            "Uniform sampling for abstract specifications of types '"
            f" '{spec_1_explicit.__class__.__name__}' and"
            f" '{spec_2_explicit.__class__.__name__}' with operation"
            f" '{self.operation}' is not supported."
        )
    ...
    def explicit(self) -> Spec:
        ...
        if isinstance(spec_1_explicit, UniverseSpec) or isinstance(
            spec_2_explicit, UniverseSpec
        ):
            ...
            if self.operation == SpecOp.UNION:
                ...
                raise NotImplementedError(
                    "Explict construction for '"
                    f" '{spec_1_explicit.__class__.__name__}' and"
                    f" '{spec_2_explicit.__class__.__name__}' with operation"
                    f" '{self.operation}' is not supported."
                )

        elif isinstance(spec_1_explicit, NullSpec) or isinstance(
            spec_2_explicit, NullSpec
        ):
        ...
        elif isinstance(spec_1_explicit, Dfa) and (
            spec_2_explicit is None or isinstance(spec_2_explicit, Dfa)
        ):
            ## All specifications are DFAs.

            if self.operation == SpecOp.UNION:
                self.explicit_form = Dfa.union_construction(
                    spec_1_explicit, spec_2_explicit
                )
            elif self.operation == SpecOp.INTERSECTION:
                self.explicit_form = Dfa.intersection_construction(
                    spec_1_explicit, spec_2_explicit
                )
            elif self.operation == SpecOp.NEGATION:
        ...
        elif isinstance(spec_1_explicit, Z3Formula) and (
            spec_2_explicit is None or isinstance(spec_2_explicit, Z3Formula)
        ):
            ## All specifications are Z3 Forumlas.

            if self.operation == SpecOp.UNION:
                self.explicit_form = Z3Formula.union_construction(
                    spec_1_explicit, spec_2_explicit
                )
            elif self.operation == SpecOp.INTERSECTION:
                self.explicit_form = Z3Formula.intersection_construction(
                    spec_1_explicit, spec_2_explicit
                )
            elif self.operation == SpecOp.NEGATION:
                self.explicit_form = spec_1_explicit.negation()
            ...
            raise NotImplementedError(
                "Explict constructions for '"
                f" '{spec_1_explicit.__class__.__name__}' and"
                f" '{spec_2_explicit.__class__.__name__}' are not supported."
            )
