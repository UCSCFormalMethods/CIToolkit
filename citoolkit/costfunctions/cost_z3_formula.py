...
class CostZ3Formula(ApproxCostFunc):
    ...
    def __init__(self, cost_var: z3.BitVecSort) -> None:
    ...
    def realize(self, min_cost: int, max_cost: int) -> Z3Formula:
        """Realize this cost function into a Z3Formula object that accepts
        only words with cost in the range [min_cost, max_cost]. Note that
        min_cost and max_cost must be integers.
        ...
        """
        if not isinstance(min_cost, int) or not isinstance(max_cost, int):
            raise ValueError("Both min_cost and max_cost must be integer values.")
