...
class _ALQCIBase(Improviser):
    ...
    def __init__(self, hard_constraint: ApproxSpec, cost_func: ApproxCostFunc, label_func: ApproxLabelFunc, \
                ...
                seed: int, num_threads: int, lazy: bool) -> None:
        ## Initialization ##
        ...
        logger.info("Beginning compound Spec construction. Using %d thread(s).", num_threads)
        ...
        logger.info("Compound Spec construction completed. Wallclock Runtime: %.4f", wall_time)
        ...
        logger.info("Desired Confidence %.4f requires counting confidence of %.4f", conf, self.counting_conf)
        ...
        if not lazy:
            ...
            if num_threads <= 1:
                ...
                for class_id in sorted(class_ids):
                    ...
                    spec.language_size(tolerance=self.counting_tol, confidence=self.counting_conf, seed=random.getrandbits(32))
            else:
                ...
                with Pool(num_threads) as pool:
                    ...
                    wrapper_inputs = [(class_id, self.class_specs[class_id].bool_formula_spec, 
                                       self.counting_tol, self.counting_conf, random_seeds[class_id]) 
                                       for class_id in class_ids]
                    ...
                    def count_wrapper(input):
                        ...
                        spec.language_size(tolerance=counting_tol, confidence=counting_conf, seed=random_seed)
...
class ALQCI(_ALQCIBase):
    ...
    def __init__(self, hard_constraint: ApproxSpec, cost_func: ApproxCostFunc, label_func: ApproxLabelFunc, \
                 bucket_ratio: float, counting_tol: float, sampling_tol: float, conf: float, \
                 seed=None, num_threads: int =1, lazy: bool =False) -> None:
        ## Initialization ##
        ...
        super().__init__(hard_constraint, cost_func, label_func, \
                         ...
                         seed, num_threads, lazy)

        logger.info("LQCI Improviser successfully initialized")

    def parameterize(self, cost_bound: float, label_prob_bounds: tuple[float, float], word_prob_bounds: dict[str, tuple[float, float]], \
        seed=None, num_threads: int =1):
        """
        ...
        for class_id in sorted(self.class_specs):
            ...
            if word_prob_bounds[label][0] == 0:
                ...
                class_count = self.class_specs[class_id].language_size(tolerance=self.counting_tol, confidence=self.counting_conf, seed=random.getrandbits(32))
                conditional_weights[class_id] = class_count * (word_prob_bounds[label][0]/(1+self.counting_tol))
        ...
        if self.lazy and num_threads > 1 and len(self.labels) > 1:
            ...
            with Pool(num_threads) as pool:
                ...
                def assign_beta(func_input):
                    label, label_specs, cond_label_weights, counting_tol, counting_conf, num_buckets, seed = func_input
                    ...
                    for bucket_iter in range(num_buckets):
                        ...
                        class_count = label_specs[bucket_iter].language_size(tolerance=counting_tol, confidence=counting_conf, seed=seed)
                        ...
                        new_cost = min((1 + counting_tol) * word_prob_bounds[label][1] * class_count, 1 - sum(cond_label_weights.values()) + cond_label_weights[bucket_iter])
                ...
                for label in sorted(self.labels):
                    ...
                    seed = random.getrandbits(32)
                    func_inputs.append((label, label_specs, cond_label_weights, self.counting_tol, self.counting_conf, self.num_buckets, seed))
                ...
                for class_id in self.class_specs:
                    label, bucket_iter = class_id
                    self.class_specs[class_id].set_bool_formula_spec(pool_dict[label][0][bucket_iter])
            ...
            for label in sorted(self.labels):
                for bucket_iter in range(self.num_buckets):
                    ...
                    conditional_weights[class_id] =  min((1 + self.counting_tol) * word_prob_bounds[label][1] * class_count, 1 - label_sum_probs[label] + conditional_weights[class_id])
        ...
        min_expected_cost = sum([marginal_weights[label] * conditional_costs[label] for label in self.labels])

        logger.info("Lower Bound on Calculated Distribution Cost: %.4f", min_expected_cost)

        wall_time = time.time() - start_time
        logger.info("ALQCI distribution calculation completed. Wallclock Runtime: %.4f", wall_time)
        ...
        self.class_keys = [(label, bucket) for label in sorted(self.labels) for bucket in range(self.num_buckets)]
