...
`   :param num_threads: The number of threads to use in parameterization computation.
    ...
    def __init__(self, hard_constraint: ExactSpec, soft_constraint: ExactSpec, label_func: ExactLabellingFunc, \
                 length_bounds: tuple[int, int], num_threads: int =1, lazy=False) -> None:
    ...
    def parameterize(self, epsilon: float, label_prob_bounds: tuple[float, float], word_prob_bounds: dict[str, tuple[float, float]], num_threads: int =1):
        ...
        try:
            super().parameterize(cost_bound, label_prob_bounds, word_prob_bounds, num_threads=num_threads)
...
`   :param num_threads: The number of threads to use in parameterization computation.
    ...
    def __init__(self, hard_constraint: ExactSpec, soft_constraint: ExactSpec, label_func: ExactLabellingFunc, \
        ...
        super().__init__(hard_constraint, cost_func, label_func, length_bounds, num_threads=num_threads)

    def parameterize(self, epsilon: float, label_prob_bounds: tuple[float, float], num_threads: int =1):
