...
def test_lqci_improvise():
    ...
    improviser = LQCI(hard_constraint, cost_func, label_func, length_bounds, cost_bound, label_prob_bounds, word_prob_bounds)
...
def test_lqci_improvise_multithreading():
    """ Test a simple Labelled Quantitative CI instance. """
    # Create a hard constraint Dfa that accepts all words start with "0"
    alphabet = {"0", "1", "2"}
    h_states = {"Start", "Accept", "Reject"}
    ...
    improviser = LQCI(hard_constraint, cost_func, label_func, length_bounds, cost_bound, label_prob_bounds, word_prob_bounds, num_threads=3)
...
def test_lqci_infeasible():
    ...
    LQCI(hard_constraint, cost_func, label_func, length_bounds, cost_bound, label_prob_bounds, word_prob_bounds)
    ...
    with pytest.raises(InfeasibleCostError):
        LQCI(hard_constraint, cost_func, label_func, length_bounds, 0.3, label_prob_bounds, word_prob_bounds)

    with pytest.raises(InfeasibleCostError):
        LQCI(hard_constraint, cost_func, label_func, length_bounds, cost_bound, (Fraction(1,3), Fraction(1,3)), word_prob_bounds)

    with pytest.raises(InfeasibleLabelRandomnessError):
        LQCI(hard_constraint, cost_func, label_func, length_bounds, cost_bound, (Fraction(1,4), Fraction(1,4)), word_prob_bounds)
    ...
    with pytest.raises(InfeasibleCostError):
        ...
        LQCI(hard_constraint, cost_func, label_func, length_bounds, cost_bound, label_prob_bounds, strict_word_prob_bounds)
    ...
    with pytest.raises(InfeasibleWordRandomnessError):
        ...
        LQCI(hard_constraint, cost_func, label_func, length_bounds, cost_bound, label_prob_bounds, infeasible_word_prob_bounds)

def test_melqci_improvise():
    """ Test a simple Labelled Quantitative CI instance. """
    # Create a hard constraint Dfa that accepts all words start with "0"
    alphabet = {"0", "1", "2"}
    h_states = {"Start", "Accept", "Reject"}
    ...
    improviser = MELQCI(hard_constraint, cost_func, label_func, length_bounds, cost_bound, label_prob_bounds)
...
def test_melqci_infeasible():
    ...
    # Ensure the base LQCI problem is feasible
    MELQCI(hard_constraint, cost_func, label_func, length_bounds, cost_bound, label_prob_bounds)

    # Check that various parameter tweaks that render the
    # problem infeasible are identified by the improviser.
    with pytest.raises(InfeasibleImproviserError):
        MELQCI(hard_constraint, cost_func, label_func, length_bounds, 0.5, label_prob_bounds)

    with pytest.raises(InfeasibleImproviserError):
        MELQCI(hard_constraint, cost_func, label_func, length_bounds, cost_bound, ((Fraction(1,4), Fraction(1,4))))
...
@pytest.mark.slow
def test_lqci_improvise_random():
...
@pytest.mark.slow
def test_melqci_improvise_random():
    """ Tests generating a mix of fully random and random
    but likely feasible Labelled Quantitative CI improviser
    instances and ensuring that they either are infeasible
    or are feasible and improvise correctly.
    """
    for _ in range(_RANDOM_LQCI_TEST_NUM_ITERS):
        # Generate random set of LQCI parameters. 50% chance to
        # generate an instance where each parameter is individually feasible.
        if random.random() < 0.5:
        ...
        try:
            ...
            LQCI(hard_constraint, cost_func, label_func, length_bounds, cost_bound, label_prob_bounds, trivial_word_bounds)
        except InfeasibleImproviserError:
            qlci_feasible = False
        else:
            qlci_feasible = True
